...
import org.zeron.sophon.authz.saml.common.EndpointGenerator;
import org.zeron.sophon.authz.saml.service.IDService;
import org.zeron.sophon.authz.saml.service.TimeService;
import org.zeron.sophon.authz.saml20.binding.decoder.OpenHTTPPostDecoder;
import org.zeron.sophon.authz.saml20.binding.decoder.OpenHTTPPostSimpleSignDecoder;
import org.zeron.sophon.authz.saml20.binding.decoder.OpenHTTPRedirectDecoder;
import org.zeron.sophon.authz.saml20.binding.impl.ExtractPostBindingAdapter;
import org.zeron.sophon.authz.saml20.binding.impl.ExtractRedirectBindingAdapter;
import org.zeron.sophon.authz.saml20.binding.impl.PostBindingAdapter;
import org.zeron.sophon.authz.saml20.binding.impl.PostSimpleSignBindingAdapter;
import org.zeron.sophon.authz.saml20.provider.xml.AuthnResponseGenerator;
import org.zeron.sophon.authz.saml20.xml.SAML2ValidatorSuite;
import org.zeron.sophon.crypto.keystore.KeyStoreLoader;
import org.zeron.sophon.entity.Saml20Metadata;
...
@ComponentScan(basePackages = {
        "org.Sophon.authz.saml20.provider.endpoint",
        "org.Sophon.authz.saml20.metadata.endpoint",
})
public class Saml20AutoConfiguration implements InitializingBean {
    ...
    AuthnResponseGenerator authnResponseGenerator(TimeService timeService, IDService idService,
                                               @Value("${sophon.saml.v20.idp.issuer}") String issuerEntityName) {
    ...
    Saml20Metadata saml20Metadata(
            @Value("${sophon.saml.v20.metadata.orgName}") String orgName,
            @Value("${sophon.saml.v20.metadata.orgDisplayName}") String orgDisplayName,
            @Value("${sophon.saml.v20.metadata.orgURL}") String orgURL,
            @Value("${sophon.saml.v20.metadata.company}") String company,
            @Value("${sophon.saml.v20.metadata.contactType}") String contactType,
            @Value("${sophon.saml.v20.metadata.givenName}") String givenName,
            @Value("${sophon.saml.v20.metadata.surName}") String surName,
            @Value("${sophon.saml.v20.metadata.emailAddress}") String emailAddress,
            @Value("${sophon.saml.v20.metadata.telephoneNumber}") String telephoneNumber) {
    ...
    ReplayCache replayCache(MapBasedStorageService<String, ReplayCacheEntry> mapBasedStorageService,
                         @Value("${sophon.saml.v20.replay.cache.life.in.millis}") long duration) {
    ...
    BasicParserPool samlParserPool(
            @Value("${sophon.saml.v20.max.parser.pool.size}") int maxPoolSize) {
    ...
    IssueInstantRule issueInstantRule(
            @Value("${sophon.saml.v20.issue.instant.check.clock.skew.in.seconds}") int newClockSkew,
            @Value("${sophon.saml.v20.issue.instant.check.validity.time.in.seconds}") int newExpires) {
    ...
    OpenHTTPPostSimpleSignDecoder openHTTPPostSimpleSignDecoder(BasicParserPool samlParserPool,
                                                             @Value("${sophon.saml.v20.idp.receiver.endpoint}") String receiverEndpoint) {
