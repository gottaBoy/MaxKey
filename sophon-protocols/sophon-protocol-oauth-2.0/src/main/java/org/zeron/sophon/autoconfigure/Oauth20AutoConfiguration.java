...
import org.zeron.sophon.authz.oauth2.common.OAuth2Constants;
import org.zeron.sophon.authz.oauth2.provider.ClientDetailsService;
import org.zeron.sophon.authz.oauth2.provider.OAuth2UserDetailsService;
import org.zeron.sophon.authz.oauth2.provider.approval.TokenApprovalStore;
import org.zeron.sophon.authz.oauth2.provider.approval.endpoint.OAuth20UserApprovalHandler;
import org.zeron.sophon.authz.oauth2.provider.client.ClientDetailsUserDetailsService;
import org.zeron.sophon.authz.oauth2.provider.client.JdbcClientDetailsService;
import org.zeron.sophon.authz.oauth2.provider.code.AuthorizationCodeServices;
import org.zeron.sophon.authz.oauth2.provider.code.InMemoryAuthorizationCodeServices;
import org.zeron.sophon.authz.oauth2.provider.code.RedisAuthorizationCodeServices;
import org.zeron.sophon.authz.oauth2.provider.endpoint.TokenEndpointAuthenticationFilter;
import org.zeron.sophon.authz.oauth2.provider.request.DefaultOAuth2RequestFactory;
import org.zeron.sophon.authz.oauth2.provider.token.DefaultTokenServices;
import org.zeron.sophon.authz.oauth2.provider.token.TokenStore;
import org.zeron.sophon.authz.oauth2.provider.token.store.InMemoryTokenStore;
import org.zeron.sophon.authz.oauth2.provider.token.store.JwtAccessTokenConverter;
import org.zeron.sophon.authz.oauth2.provider.token.store.RedisTokenStore;
import org.zeron.sophon.authz.oidc.idtoken.OIDCIdTokenEnhancer;
import org.zeron.sophon.configuration.oidc.OIDCProviderMetadataDetails;
import org.zeron.sophon.constants.ConstsPersistence;
import org.zeron.sophon.crypto.jose.keystore.JWKSetKeyStore;
import org.zeron.sophon.crypto.jwt.encryption.service.impl.DefaultJwtEncryptionAndDecryptionService;
import org.zeron.sophon.crypto.jwt.signer.service.impl.DefaultJwtSigningAndValidationService;
...
@ComponentScan(basePackages = {
        "org.zeron.sophon.authz.oauth2.provider.endpoint",
        "org.zeron.sophon.authz.oauth2.provider.userinfo.endpoint",
        "org.zeron.sophon.authz.oauth2.provider.approval.controller",
        "org.zeron.sophon.authz.oauth2.provider.wellknown.endpoint"
})
public class Oauth20AutoConfiguration implements InitializingBean {
    ...
    OIDCProviderMetadataDetails oidcProviderMetadata(
            @Value("${sophon.oidc.metadata.issuer}")
            String issuer,
            @Value("${sophon.oidc.metadata.authorizationEndpoint}")
            URI authorizationEndpoint,
            @Value("${sophon.oidc.metadata.tokenEndpoint}")
            URI tokenEndpoint,
            @Value("${sophon.oidc.metadata.userinfoEndpoint}")
    ...
    DefaultJwtSigningAndValidationService jwtSignerValidationService(
        ...
        jwtSignerValidationService.setDefaultSignerKeyId("Sophon_rsa");
    ...
    DefaultJwtEncryptionAndDecryptionService jwtEncryptionService(
        ...
        jwtEncryptionService.setDefaultDecryptionKeyId("Sophon_rsa");
        jwtEncryptionService.setDefaultEncryptionKeyId("Sophon_rsa");
