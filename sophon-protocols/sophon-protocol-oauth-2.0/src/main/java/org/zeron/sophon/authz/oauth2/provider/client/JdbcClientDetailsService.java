...
package org.zeron.sophon.authz.oauth2.provider.client;
...
import org.zeron.sophon.authz.oauth2.common.util.DefaultJdbcListFactory;
import org.zeron.sophon.authz.oauth2.common.util.JdbcListFactory;
import org.zeron.sophon.authz.oauth2.provider.ClientAlreadyExistsException;
import org.zeron.sophon.authz.oauth2.provider.ClientDetailsService;
import org.zeron.sophon.authz.oauth2.provider.ClientRegistrationService;
import org.zeron.sophon.authz.oauth2.provider.NoSuchClientException;
...
import org.zeron.sophon.entity.apps.oauth2.provider.client.BaseClientDetails;
...
public class JdbcClientDetailsService implements ClientDetailsService, ClientRegistrationService{
    ...
    private static final String BASE_FIND_STATEMENT = "select client_id, " + CLIENT_FIELDS
            + " from ze_apps_oauth_client_details";

    private static final String DEFAULT_FIND_STATEMENT = BASE_FIND_STATEMENT + " order by client_id";

    private static final String DEFAULT_SELECT_STATEMENT = BASE_FIND_STATEMENT + " where client_id = ?";

    private static final String DEFAULT_INSERT_STATEMENT = "insert into ze_apps_oauth_client_details (" + CLIENT_FIELDS
            + ", client_id) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

    private static final String DEFAULT_UPDATE_STATEMENT = "update ze_apps_oauth_client_details " + "set "
            + CLIENT_FIELDS_FOR_UPDATE.replaceAll(", ", "=?, ") + "=? where client_id = ?";

    private static final String DEFAULT_UPDATE_SECRET_STATEMENT = "update ze_apps_oauth_client_details "
            + "set client_secret = ? where client_id = ?";

    private static final String DEFAULT_DELETE_STATEMENT = "delete from ze_apps_oauth_client_details where client_id = ?";
