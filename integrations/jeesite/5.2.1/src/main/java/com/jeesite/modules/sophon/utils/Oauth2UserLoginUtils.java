package com.jeesite.modules.Sophon.utils;
...
public class Oauth2UserLoginUtils{
    ...
    try {
        // FormToken 构造方法的三个参数：登录名、是否内部登录无条件、请求对??
...
//                serverUrl: http://sso.Sophon.top
//                redirectUri: http://localhost:8980/js/oauth2/callback/jeegit
//                className: com.jeesite.modules.oauth2.request.AuthSophonRequest                  
            authRequest = new AuthSophonJeeGitRequest(AuthConfig.builder()
...
//                        .redirectUri("http://localhost:9001/oauth/baidu/callback")
            ...
            case "oschina":
                ...
                break;
            case "alipay":
                // 支付宝在创建回调地址时，不允许使用localhost或�?27.0.0.1，所以这儿的回调地址使用的局域网内的ip
