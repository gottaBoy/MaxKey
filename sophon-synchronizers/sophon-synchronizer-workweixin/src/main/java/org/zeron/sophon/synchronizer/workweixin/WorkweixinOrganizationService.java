...
import org.zeron.sophon.synchronizer.workweixin.entity.WorkWeixinDepts;
import org.zeron.sophon.synchronizer.workweixin.entity.WorkWeixinDeptsResponse;
...
public class WorkweixinOrganizationService extends AbstractSynchronizerService implements ISynchronizerService {
    ...
    public void sync() {
        ...
        try {
            ...
            // 需要对企业微信部门列表进行一次重排，保证父节点在前，子节点在??
            ...
            // 从映射里面拿到企业微信Id映射后的本地组织的字??用于判断本地的组织是否存??
            ...
            for (WorkWeixinDepts deptWxCur : deptWxListAfterLevelSort) {
                ...
                if (deptWxCur.getId() == ROOT_DEPT_ID) {
                    ...
                    // 现在不是根组??
                    //synchro Related 查询当前部门是否有同步记??这里只是查有没有关系, 不是查组??
                    ...
                    //Parent 查询当前部门父部门是否有同步记录 这里只是查有没有关系, 不是查组??
                    ...
                    // 这里需要修正一下层级关?? 防止因为映射关系错误导致的层级错??
                    String deptWxParentId = String.valueOf(deptWxCur.getParentid());
                    // 进入到这个节点的应该都是有上级的, 现在只需要根据上级Id查询上级的组织档??
    ...
    public SynchroRelated buildSynchroRelated(Organizations organization, WorkWeixinDepts dept) {
        return new SynchroRelated(
                ...
                organization.getOrgName(), // objectName 系统内组织名??
                organization.getOrgName(), // objectDisplayName 系统内组织显示名??
                Organizations.CLASS_TYPE, // objectType 对象类型
                synchronizer.getId(), // syncId 同步器ID
                synchronizer.getName(), // syncName 同步器名??
     ...
     *
     * @param dept                 企业微信部门实体
     * @param synchroRelatedParent 父部门同步关??
