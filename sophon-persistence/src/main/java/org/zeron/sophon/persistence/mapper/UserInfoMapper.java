...
public interface UserInfoMapper  extends IJpaMapper<UserInfo>{
    ...
    @Select("select * from  ze_userinfo where username = #{value} and status = " + ConstsStatus.ACTIVE)
    public UserInfo findByUsername(String username);
    
    @Select("select * from  ze_userinfo where ( email = #{value} or mobile= #{value} ) and status = " + ConstsStatus.ACTIVE)
    ...
    @Update("update ze_userinfo set gridlist =  #{gridList} where id = #{id}")
    public int     updateGridList(UserInfo userInfo) ;
    
    @Update("update ze_userinfo set status =  #{status} where id = #{id}")
