...
public interface PasskeyChallengeMapper extends IJpaMapper<PasskeyChallenge> {
     ...
     */
    @Select("SELECT * FROM ze_passkey_challenges WHERE id = #{challengeId}")
    @Results({
     ...
     * @return PasskeyChallenge对象
     */
    @Select("SELECT * FROM ze_passkey_challenges WHERE user_id = #{userId} AND challenge_type = #{challengeType} ORDER BY created_date DESC LIMIT 1")
    @Results({
        @Result(column = "id", property = "id"),
        @Result(column = "user_id", property = "userId"),
        @Result(column = "challenge", property = "challenge"),
     ...
     * @return 删除的记录数
     */
    @Delete("DELETE FROM ze_passkey_challenges WHERE id = #{challengeId}")
     ...
     * @return 清理的记录数
     */
    @Delete("DELETE FROM ze_passkey_challenges WHERE expires_date < NOW()")
     ...
     * @return 清理的记录数
     */
    @Delete("DELETE FROM ze_passkey_challenges WHERE user_id = #{userId}")
     ...
     * @return 清理的记录数
     */
    @Delete("DELETE FROM ze_passkey_challenges WHERE user_id = #{userId} AND challenge_type = #{challengeType}")
     ...
     * @return 挑战数量
     */
    @Select("SELECT COUNT(*) FROM ze_passkey_challenges WHERE user_id = #{userId}")
     ...
     * @return 是否存在且有??
     */
    @Select("SELECT COUNT(*) > 0 FROM ze_passkey_challenges WHERE id = #{challengeId} AND expires_date > NOW()")
