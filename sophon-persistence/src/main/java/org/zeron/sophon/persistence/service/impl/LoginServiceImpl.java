...
public class LoginServiceImpl  implements LoginService{
    ...
    private static final String LOGIN_USERINFO_UPDATE_STATEMENT = "update ze_userinfo set badpasswordcount = 0 , logincount = logincount + 1 , lastlogintime = ?  , lastloginip = ? , online = "
            + UserInfo.ONLINE.ONLINE + "  where id = ?";

    private static final String GROUPS_SELECT_STATEMENT = "select distinct g.id,g.groupcode,g.groupname from ze_userinfo u,ze_groups g,ze_group_member gm where u.id = ?  and u.id=gm.memberid and gm.groupid=g.id ";

    private static final String DEFAULT_USERINFO_SELECT_STATEMENT = "select * from  ze_userinfo where username = ? ";
    
    private static final String DEFAULT_USERINFO_SELECT_STATEMENT_BY_ID = "select * from  ze_userinfo where id = ? ";

    private static final String DEFAULT_USERINFO_SELECT_STATEMENT_USERNAME_MOBILE = "select * from  ze_userinfo where (username = ? or mobile = ?)";

    private static final String DEFAULT_USERINFO_SELECT_STATEMENT_USERNAME_MOBILE_EMAIL = "select * from  ze_userinfo where (username = ? or mobile = ? or email = ?) ";

    private static final String DEFAULT_MYAPPS_SELECT_STATEMENT = "select distinct app.id,app.appname from ze_apps app,ze_access gp,ze_groups g  where app.id=gp.appid and app.status = 1 and gp.groupid=g.id and g.id in(%s)";
