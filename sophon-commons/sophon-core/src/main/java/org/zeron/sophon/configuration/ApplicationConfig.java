...
package org.zeron.sophon.configuration;
 ...
 * 
 * 其中1????项在applicationContext.xml中配置，配置文件applicationConfig.properties
 * 4项根据dynamic的属性判断是否动态从sysConfigService动态读??
 * 
 * @author yi.min
 * 
 */
...
public class ApplicationConfig {
    ...
    @Value("${sophon.server.basedomain}")
    String baseDomainName;

    @Value("${sophon.server.domain}")
    String domainName;

    @Value("${sophon.server.name}")
    String serverName;

    @Value("${sophon.server.uri}")
    String serverPrefix;

    @Value("${sophon.server.default.uri}")
    String defaultUri;

    @Value("${sophon.server.mgt.uri}")
    String mgtUri;
    
    @Value("${sophon.server.authz.uri}")
    private String authzUri;
    
    @Value("${sophon.server.frontend.uri:http://sso.Sophon.top:4200}")
    private String frontendUri;

    @Value("${server.port:8080}")
    private int port;
    
    @Value("${sophon.server.provision:false}")
    private boolean provision;
    
    @Value("${sophon.server.persistence}") 
    int persistence;
    
    @Value("${sophon.notices.visible:false}")
