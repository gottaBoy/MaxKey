...
package org.zeron.sophon.passkey.endpoint;

import org.zeron.sophon.passkey.manager.PasskeyManager;
...
public class PasskeyAuthenticationEndpoint {
    ...
    /**
     * 创建完整的登录会??
     * 参见 AbstractAuthenticationProvider.createOnlineTicket 方法
     * @param userInfo 用户信息
     * @return 认证令牌
     */
    private UsernamePasswordAuthenticationToken createOnlineTicket(UserInfo userInfo) {
        try {
            ...
            loginCredential.setPassword(""); // Passkey认证不需要密??
            ...
            if (userId == null || userId.isEmpty()) {
                ...
                if (!hasPasskeys) {
                    ...
                    return ResponseEntity.badRequest()
                        .body(new Message<>(Message.ERROR, "系统中还没有注册任何 Passkey，请先注册Passkey 后再使用此功能"));
                } else {
    ...
    public ResponseEntity<?> finishAuthentication(
        ...
        try {
            ...
            if (result != null && Boolean.TRUE.equals(result.get("success"))) {
                ...
                // 关键修改：返回完整的 AuthJwt 对象，而不是只返回 token 字符??
                if (authJwtObj != null) {
                    // 直接返回完整??AuthJwt 对象，包含所有必要的字段
                    ...
                    responseData.put("authorities", authJwtObj.getAuthorities()); // 关键：包含权限信??
                    ...
                    responseData.put("authorities", new ArrayList<>()); // 空权限列??
                }
                ...
                if (redirectUrl != null) {
                    ...
                    responseData.put("redirectUrl", "/index"); // 默认跳转到首??
    ...
    public ResponseEntity<?> getAuthenticationStatus(
        ...
        try {
            ...
            return ResponseEntity.internalServerError()
                .body(new Message<>(Message.ERROR, "获取认证状态失?? " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> verifyAuthentication(
        ...
        try {
            ...
            return ResponseEntity.internalServerError()
                .body(new Message<>(Message.ERROR, "验证认证状态失?? " + e.getMessage()));
        }
    }
}
