...
import org.zeron.sophon.crypto.password.LdapShaPasswordEncoder;
import org.zeron.sophon.crypto.password.Md4PasswordEncoder;
import org.zeron.sophon.crypto.password.MessageDigestPasswordEncoder;
import org.zeron.sophon.crypto.password.NoOpPasswordEncoder;
import org.zeron.sophon.crypto.password.PasswordReciprocal;
import org.zeron.sophon.crypto.password.SM3PasswordEncoder;
import org.zeron.sophon.crypto.password.StandardPasswordEncoder;
import org.zeron.sophon.persistence.cache.InMemoryMomentaryService;
import org.zeron.sophon.persistence.cache.MomentaryService;
import org.zeron.sophon.persistence.cache.RedisMomentaryService;
import org.zeron.sophon.persistence.redis.RedisConnectionFactory;
import org.zeron.sophon.util.IdGenerator;
import org.zeron.sophon.util.SnowFlakeId;
...
public class ApplicationAutoConfiguration {
    ...
    PasswordEncoder passwordEncoder(
            @Value("${sophon.crypto.password.encoder:bcrypt}") String idForEncode) {
    ...
    KeyStoreLoader keyStoreLoader(
            @Value("${sophon.saml.v20.idp.issuing.entity.id}") String entityName,
            @Value("${sophon.saml.v20.idp.keystore.password}") String keystorePassword,
            @Value("${sophon.saml.v20.idp.keystore}") Resource keystoreFile) {
    ...
    KeyStoreLoader serviceProviderKeyStoreLoader(
            @Value("${sophon.saml.v20.sp.issuing.entity.id}") String entityName,
            @Value("${sophon.saml.v20.sp.keystore.password}") String keystorePassword,
            @Value("${sophon.saml.v20.sp.keystore}") Resource keystoreFile) {
        KeyStoreLoader keyStoreLoader = new KeyStoreLoader();
        keyStoreLoader.setEntityName(entityName);
        keyStoreLoader.setKeystorePassword(keystorePassword);
        keyStoreLoader.setKeystoreFile(keystoreFile);
    ...
    String spIssuingEntityName(
            @Value("${sophon.saml.v20.sp.issuing.entity.id}") String spIssuingEntityName) {
