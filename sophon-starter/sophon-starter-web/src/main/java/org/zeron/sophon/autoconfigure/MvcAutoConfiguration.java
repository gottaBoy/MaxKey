...
import org.zeron.sophon.web.WebInstRequestFilter;
import org.zeron.sophon.web.WebXssRequestFilter;
...
public class MvcAutoConfiguration implements WebMvcConfigurer {
    ...
    /**
     * 消息处理，可以直接使用properties的key值，返回的是对应的value??
     * messageSource .
    ...
    Jaxb2Marshaller jaxb2Marshaller() {
        ...
        jaxb2Marshaller.setClassesToBeBound(org.zeron.sophon.entity.xml.UserInfoXML.class);
        return jaxb2Marshaller;
    }
    ...
    LocaleResolver cookieLocaleResolver(
            @Value("${sophon.server.domain:Sophon.top}")
            String domainName
    ) {
        _logger.debug("DomainName {}" , domainName);
        CookieLocaleResolver cookieLocaleResolver = new CookieLocaleResolver("ze_locale");
    ...
    RequestMappingHandlerAdapter requestMappingHandlerAdapter(
        ...
        //需要追加byte，否则springdoc-openapi接口会响应Base64编码内容，导致接口文档显示失??
    ...
    /**
     * 配置默认错误页面（仅用于内嵌tomcat启动时） 使用这种方式，在打包为war后不起作??
     *
